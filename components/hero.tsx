...
export function Hero() {
  ...
  const githubLink = contactData.socialLinks.find((link) => link.name === "GitHub")?.href ?? "https://github.com";
  const linkedinLink = contactData.socialLinks.find((link) => link.name === "LinkedIn")?.href ?? "https://linkedin.com";
        ...
        >
          ...
          <div className="flex gap-4">
            <motion.a
              ...
              whileTap={{ scale: 0.96 }}
              className="group relative inline-flex cursor-pointer items-center justify-center rounded-xl border border-border bg-background/60 p-3 text-foreground shadow-sm transition-all duration-200 hover:border-primary/60 hover:bg-accent hover:shadow-[0_0_24px_rgba(59,130,246,0.18)]"
            >
              <span className="absolute inset-0 rounded-xl bg-primary/10 opacity-0 blur-md transition-opacity duration-200 group-hover:opacity-100" />
              <svg className="relative h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
